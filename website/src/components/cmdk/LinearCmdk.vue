...
const items = [
  { label: 'Assign to...', shortcut: ['A'] },
  { label: 'Assign to me', shortcut: ['I'] },
  { label: 'Change status...', shortcut: ['S'] },
  { label: 'Change priority...', shortcut: ['P'] },
  { label: 'Change labels...', shortcut: ['L'] },
  { label: 'Remove label...', shortcut: ['⇧', 'L'] },
  { label: 'Set due date...', shortcut: ['⇧', 'D'] },
]
</script>
  ...
  <div class="linear">
    <Command>
      ...
      <Command.List>
        ...
        <Command.Item v-for="item in items" :key="item.label" :value="item.label">
          <AssignToIcon v-if="item.label === 'Assign to...'" />
          <AssignToMeIcon v-else-if="item.label === 'Assign to me'" />
          <ChangeStatusIcon v-else-if="item.label === 'Change status...'" />
          <ChangePriorityIcon v-else-if="item.label === 'Change priority...'" />
          <ChangeLabelsIcon v-else-if="item.label.startsWith('Change labels')" />
          <RemoveLabelIcon v-else-if="item.label.startsWith('Remove label')" />
          <SetDueDateIcon v-else />
          {{ item.label }}
...
const AssignToIcon = defineComponent({
  render: () =>
...
const AssignToMeIcon = defineComponent({
  render: () =>
    h('svg', { width: '16', height: '16', viewBox: '0 0 16 16', fill: 'currentColor' }, [
      h('path', {
...
const ChangeStatusIcon = defineComponent({
  render: () =>
    h('svg', { width: '16', height: '16', viewBox: '-1 -1 15 15', fill: 'currentColor' }, [
      h('path', {
...
const ChangePriorityIcon = defineComponent({
  render: () =>
    h('svg', { width: '16', height: '16', viewBox: '0 0 16 16', fill: 'currentColor' }, [
      h('rect', { x: '1', y: '8', width: '3', height: '6', rx: '1' }),
      h('rect', { x: '6', y: '5', width: '3', height: '9', rx: '1' }),
      h('rect', { x: '11', y: '2', width: '3', height: '12', rx: '1' }),
    ]),
})
const ChangeLabelsIcon = defineComponent({
  render: () =>
    h('svg', { width: '16', height: '16', viewBox: '0 0 16 16', fill: 'currentColor' }, [
      h('path', {
        'fill-rule': 'evenodd',
...
const RemoveLabelIcon = ChangeLabelsIcon
const SetDueDateIcon = defineComponent({
  render: () =>
    h('svg', { width: '16', height: '16', viewBox: '0 0 16 16', fill: 'currentColor' }, [
      h('path', {
        'fill-rule': 'evenodd',
